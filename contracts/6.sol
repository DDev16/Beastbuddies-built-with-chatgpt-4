...
// Helper function to convert item type to its corresponding string name
function getItemName(uint256 itemType) public view returns (string memory) {
    require(itemType > 0 && itemType < itemNames.length, "Invalid item type");
    return itemNames[itemType - 1];
}
...
// Add a function to view all items in a player's inventory
function viewPlayerItems(address player) public view returns (string[] memory) {
    uint256[] memory itemTypes = getPlayerItemTypes(player);
    string[] memory items = new string[](itemTypes.length);

    for (uint256 i = 0; i < itemTypes.length; i++) {
        items[i] = getItemName(itemTypes[i]);
    }
    return items;
}
...
function viewBattleItemsAsString(uint256 battleId, address player) public view returns (string[] memory) {
    uint256[] memory itemTypes = getBattleItemTypes(battleId, player);
    string[] memory items = new string[](itemTypes.length);
...
// Function to get the item types in a battle for a specific player
function getBattleItemTypes(uint256 battleId, address player) public view returns (uint256[] memory) {
    uint256[] memory itemTypes = new uint256[](battleItems[battleId][player].length);

    for (uint256 i = 0; i < battleItems[battleId][player].length; i++) {
        itemTypes[i] = uint256(battleItems[battleId][player][i].itemType);
    }
    return itemTypes;
}
...
// Update the buyShield, buyPotion, and buyPowerUp functions to add items to an array
...
function buyPotion() public {
    uint256 cost = 200; // Set cost for Potion
    uint256 effectValue = 20; // Set potion effect value
    require(!isInActiveBattle(msg.sender), "Cannot buy items during a battle");
...
function buyPowerUp() public {
    uint256 cost = 300; // Set cost for PowerUp
    uint256 effectValue = 5; // Set power-up effect value
    require(!isInActiveBattle(msg.sender), "Cannot buy items during a battle");
